...
export class FloorChunck extends THREE.Group {
    // the visible plane mesh (fully transparent) and the outline line
    public plane: THREE.Mesh;
    public outline: THREE.Line | THREE.LineLoop;
...
export class Floor extends THREE.Group {
    ...
    public chuncks: FloorChunck[] = [];
    public size: number; // size of one chunck in world units
    public grid: THREE.Vector2; // logical grid coordinates of this floor (in chuncks)

    constructor(gridX: number, gridZ: number, size: number) {
        super();
        this.size = size;
        ...
        for (let dz = 0; dz < 2; dz++) {
            for (let dx = 0; dx < 2; dx++) {
                ...
                chunck.position.set(
                    (this.grid.x * 2 + dx) * size,
                    0,
                    (this.grid.y * 2 + dz) * size
                );
